...
class MetricsCollector:
    ...
    def get_rate(self, name: str, window_seconds: int = 60, labels: Optional[Dict[str, str]] = None) -> float:
        """
    ...
    def get_metrics_for_export(self, metric_names: Optional[List[str]] = None) -> List[MetricValue]:
        """
        ...
        with self._lock:
            if metric_names:
                return [
                    metric for name, metrics in self._metrics.items()
                    if name in metric_names
                    for metric in metrics[-100:]  # Last 100 values per metric
                ]
            else:
                return [
                    metric for metrics in self._metrics.values()
                    for metric in metrics[-100:]  # Last 100 values per metric
                ]
