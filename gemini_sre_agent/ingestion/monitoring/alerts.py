        ...
        for rule_name, rule in self._alert_rules.items():
            ...
            try:
                if rule.condition(data):
                    ...
                    # Set cooldown
                    self._suppressed_alerts[f"rule_{rule_name}"] = datetime.now() + timedelta(minutes=rule.cooldown_minutes)
        ...
        while self._running:
            try:
                ...
                self._suppressed_alerts = {
                    key: value for key, value in self._suppressed_alerts.items()
...
async def create_health_alert(component: str, message: str, level: AlertLevel = AlertLevel.WARNING) -> Alert:
...
async def create_performance_alert(component: str, message: str, level: AlertLevel = AlertLevel.WARNING) -> Alert:
...
async def create_error_alert(component: str, error: Exception, level: AlertLevel = AlertLevel.CRITICAL) -> Alert:
