...
class AnalyticsConfig(BaseModel):
    ...
    cost_optimization_threshold: float = Field(
        0.1, gt=0, description="Cost difference threshold for optimization recommendations"
    )
    performance_weight: float = Field(0.3, ge=0, le=1, description="Weight for performance in cost optimization")
    quality_weight: float = Field(0.4, ge=0, le=1, description="Weight for quality in cost optimization")
...
class CostAnalytics:
    ...
    def get_cost_trends(
        ...
        for period, records in grouped_records.items():
            ...
            if previous_period_cost is not None:
                cost_change_percent = ((total_cost - previous_period_cost) / previous_period_cost) * 100
                previous_request_count = len(grouped_records.get(list(grouped_records.keys())[-2], []))
                if previous_request_count > 0:
                    request_change_percent = ((request_count - previous_request_count) / previous_request_count) * 100
    ...
    def get_provider_comparison(
        ...
        for provider, records in provider_records.items():
            ...
            cost_efficiency_score = (success_rate / 100) / (avg_cost_per_request + 0.001)  # Avoid division by zero
    ...
    def _analyze_provider_switches(self, records: List[UsageRecord]) -> List[OptimizationRecommendation]:
        ...
        for model_type, provider_records in model_groups.items():
            ...
            alternatives = [
                (provider, data) for provider, data in provider_costs.items()
                if (provider != expensive_provider[0] and 
                    data['avg_cost'] < expensive_provider[1]['avg_cost'] * 0.8 and  # 20% cheaper
                    data['success_rate'] >= expensive_provider[1]['success_rate'] * 0.95)  # Similar success rate
            ]
    ...
    def _analyze_model_optimization(self, records: List[UsageRecord]) -> List[OptimizationRecommendation]:
        ...
        for provider, model_records in provider_groups.items():
            ...
            for model, model_recs in model_records.items():
                ...
                avg_response_time = sum(r.response_time_ms for r in model_recs if r.response_time_ms) / len([r for r in model_recs if r.response_time_ms])
            ...
            for model, data in model_analysis.items():
                ...
                alternatives = [
                    (alt_model, alt_data) for alt_model, alt_data in model_analysis.items()
                    if (alt_model != model and 
                        alt_data['avg_cost'] < data['avg_cost'] * 0.7 and  # 30% cheaper
                        alt_data['success_rate'] >= data['success_rate'] * 0.9)  # Similar success rate
                ]
                
                if alternatives:
                    best_alternative = min(alternatives, key=lambda x: x[1]['avg_cost'])
                    potential_savings = (data['avg_cost'] - best_alternative[1]['avg_cost']) * data['request_count']
    ...
    def _analyze_usage_patterns(self, records: List[UsageRecord]) -> List[OptimizationRecommendation]:
        ...
        if hourly_usage:
            ...
            if peak_cost > off_peak_cost * 2:  # Significant peak usage
                recommendations.append(OptimizationRecommendation(
                    type="usage_pattern",
                    priority="low",
                    potential_savings=peak_cost * 0.1,  # Estimate 10% savings from load balancing
                    confidence=0.6,
    ...
    def get_cost_summary(
        ...
        for record in filtered_records:
            ...
            if provider not in provider_breakdown:
                provider_breakdown[provider] = {'cost': 0, 'requests': 0, 'successful': 0}
