...
class BudgetManager:
    ...
    def _enforce_budget_limits(self) -> None:
        ...
        if current_spend > effective_budget:
            if self.config.enforcement_policy == EnforcementPolicy.HARD_LIMIT:
                logger.error(f"Budget exceeded! Current spend: ${current_spend:.2f}, Limit: ${effective_budget:.2f}")
                ...
                logger.warning(f"Budget exceeded (soft limit). Current spend: ${current_spend:.2f}, Limit: ${effective_budget:.2f}")
    ...
    def can_make_request(self, estimated_cost: float) -> tuple[bool, str]:
        ...
        if total_after_request > effective_budget:
            if self.config.enforcement_policy == EnforcementPolicy.HARD_LIMIT:
                return False, f"Request would exceed budget limit (${total_after_request:.2f} > ${effective_budget:.2f})"
            elif self.config.enforcement_policy == EnforcementPolicy.SOFT_LIMIT:
                return True, f"Warning: Request would exceed budget limit (${total_after_request:.2f} > ${effective_budget:.2f})"
    ...
    def get_budget_forecast(self, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            "effective_budget": self.get_effective_budget(),
            "forecast": forecast,
            "days_until_budget_exhausted": int(self.get_effective_budget() / daily_rate) if daily_rate > 0 else None
        }
