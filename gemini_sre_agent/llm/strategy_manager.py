...
class ModelSelectionStrategy(ABC):
    ...
    def __init__(self, name: str, model_scorer: Optional[ModelScorer] = None):
        ...
        self._selection_history: List[Tuple[Optional[ModelInfo], float, bool]] = []  # (model, score, success)
    ...
    def update_performance(self, model: Optional[ModelInfo], score: float, success: bool, latency_ms: float):
        ...
        self._performance_metrics["average_score"] = alpha * score + (1 - alpha) * current_avg
        
        # Update average latency
        current_latency = self._performance_metrics["average_latency"]
        self._performance_metrics["average_latency"] = alpha * latency_ms + (1 - alpha) * current_latency
    ...
    def _filter_candidates(self, candidates: List[ModelInfo], context: StrategyContext) -> List[ModelInfo]:
...
class CostOptimizedStrategy(ModelSelectionStrategy):
    ...
    def select_model(self, candidates: List[ModelInfo], context: StrategyContext) -> StrategyResult:
        ...
        scoring_context = ScoringContext(
            ...
            provider_preference=context.provider_preference[0] if context.provider_preference else None,
        )
...
class QualityOptimizedStrategy(ModelSelectionStrategy):
    ...
    def select_model(self, candidates: List[ModelInfo], context: StrategyContext) -> StrategyResult:
        ...
        selected_model, score = max(scored_models, key=lambda x: x[1].dimension_scores.get('quality', x[1].dimension_scores.get('performance', 0)))
        ...
        fallback_models = sorted(
            [(m, s) for m, s in scored_models if m != selected_model],
            key=lambda x: x[1].dimension_scores.get('quality', x[1].dimension_scores.get('performance', 0)),
            reverse=True
        )[:3]
        fallback_models = [m for m, s in fallback_models]
        
        quality_score = score.dimension_scores.get('quality', score.dimension_scores.get('performance', 0))
...
class HybridStrategy(ModelSelectionStrategy):
    ...
    def select_model(self, candidates: List[ModelInfo], context: StrategyContext) -> StrategyResult:
        ...
        if context.custom_weights:
            ...
            weights = self._learning_weights if self._performance_metrics["total_selections"] > 10 else ScoringWeights(
                cost=0.25,
                performance=0.3,
                reliability=0.25,
                speed=0.1,
...
class StrategyManager:
    ...
    def select_model(
        ...
        strategy_metrics["average_latency"] = (
            (strategy_metrics["average_latency"] * (strategy_metrics["total_selections"] - 1) + result.execution_time_ms) /
            strategy_metrics["total_selections"]
        )
        
        logger.debug(f"Selected model {result.selected_model.name} using {goal.value} strategy in {result.execution_time_ms:.2f}ms")
    ...
    def update_strategy_performance(self, goal: OptimizationGoal, success: bool, latency_ms: float):
        ...
        logger.debug(f"Updated performance for {goal.value} strategy: success={success}, latency={latency_ms}ms")
