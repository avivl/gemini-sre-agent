...
from .model_selector import ModelSelector, SelectionCriteria, SelectionStrategy, SelectionResult
from .model_scorer import ModelScorer, ScoringWeights, ScoringContext
from .performance_cache import PerformanceMonitor, MetricType
...
class EnhancedLLMService(Generic[T]):
    ...
    def __init__(
        ...
        self.logger.info("EnhancedLLMService initialized with intelligent model selection")
        ...
        **kwargs: Any
        ...
        try:
            ...
            if provider_name not in self.providers:
                raise ValueError(f"Provider '{provider_name}' not available for model '{selected_model.name}'")
            
            provider_instance = self.providers[provider_name]
            
            self.logger.info(f"Generating structured response using model: {selected_model.name} via provider: {provider_name}")
                ...
                if 'selected_model' in locals() and 'selected_model' in locals():
                    ...
                    if selected_model:
                        self.performance_monitor.record_success(
                            ...
                            success=False,
                            provider=selected_model.provider,
                            context={'task_type': 'structured_generation', 'error': str(e)}
                        )
            except Exception:
    ...
    async def generate_text(
        ...
        **kwargs: Any
        ...
        try:
            ...
            self.logger.info(f"Generating text response using model: {selected_model.name} via provider: {provider_name}")
    ...
    async def generate_with_fallback(
        ...
        **kwargs: Any
        ...
        for i, model_info in enumerate(selection_result.fallback_chain):
            ...
            try:
                self.logger.info(f"Attempting model: {model_info.name} (attempt {i+1}/{max_attempts})")
                
                # Get the provider for this model
                provider_name = model_info.provider.value
                if provider_name not in self.providers:
                    self.logger.warning(f"Provider '{provider_name}' not available for model '{model_info.name}'")
                    continue
                ...
                self.performance_monitor.record_success(
                    ...
                    success=False,
                    provider=model_info.provider,
                    context={'task_type': 'fallback_generation', 'attempt': i+1, 'error': str(e)}
                )
                
                continue
        
        # All models failed
        self.logger.error(f"All {max_attempts} models in fallback chain failed. Last error: {str(last_error)}")
