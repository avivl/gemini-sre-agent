...
class PerformanceCache:
    ...
    def __init__(
        ...
        self.logger.info(f"PerformanceCache initialized with max_size={max_cache_size}, ttl={default_ttl}s")
    ...
    def add_metric(self, metric: PerformanceMetric) -> None:
        ...
        self.logger.debug(f"Added metric: {metric.metric_type.value}={metric.value} for {metric.model_name}")
        ...
        for model_name, stats in self._model_stats.items():
            ...
            for metric_type, weight in weights.items():
                ...
                if avg_score is not None:
                    ...
                    normalized_score = min(1.0, max(0.0, avg_score / 1000.0))  # Adjust based on expected range
    ...
    def _cleanup_expired_metrics(self) -> None:
        ...
        # Update time index
        self._time_index = [
            (timestamp, idx) for timestamp, idx in self._time_index
