...
try:
    ...
    try:
        ...
        from mirascope.types import Message
    except ImportError:
        ChatPrompt = None
        Message = None
    ...
    try:
        ...
        from mirascope.google import GoogleCall
    except ImportError:
        AnthropicCall = None
        OpenAICall = None
        GoogleCall = None
...
class EnhancedPromptManager:
    ...
    def record_usage(
        ...
        if "response_time" in metrics:
            version_metrics.avg_response_time = (
                (version_metrics.avg_response_time * version_metrics.total_requests + metrics["response_time"]) /
        ...
        if "cost" in metrics:
            ...
            version_metrics.avg_cost_per_request = version_metrics.total_cost / (version_metrics.total_requests + 1)
        
        if "quality_score" in metrics:
            version_metrics.avg_quality_score = (
                (version_metrics.avg_quality_score * version_metrics.total_requests + metrics["quality_score"]) /
                (version_metrics.total_requests + 1)
            )
    ...
    def get_analytics(
        ...
        if version:
            ...
            version_data = [
                record for record in self.analytics_data.get(prompt_id, [])
                if record["version"] == version and 
