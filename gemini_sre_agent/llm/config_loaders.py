...
class EnvironmentConfigLoader(BaseConfigLoader):
    ...
    def _set_nested_value(self, data: Dict[str, Any], key_path: str, value: Any) -> None:
        ...
        if key_path.endswith('_enabled') or key_path.endswith('_fallback') or key_path.endswith('_monitoring'):
            current[keys[-1]] = value.lower() in ('true', '1', 'yes', 'on')
        elif key_path.endswith('_budget') or key_path.endswith('_threshold') or key_path.endswith('_timeout'):
            try:
    ...
    def _load_provider_env_vars(self) -> Dict[str, Any]:
        ...
        for provider in ['openai', 'anthropic', 'xai', 'ollama', 'bedrock', 'gemini', 'claude', 'grok']:
    ...
    def load_all(self) -> Dict[str, Any]:
        ...
        for loader in sorted_loaders:
            try:
                ...
                self._results.append(LoaderResult(
                    data={},
