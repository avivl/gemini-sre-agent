...
from gemini_sre_agent.llm.model_registry import ModelRegistry, ModelInfo, ModelCapability
from gemini_sre_agent.llm.model_scorer import ModelScorer, ScoringWeights, ModelScore, ScoringContext
...
class TestModelSelector:
    ...
    def test_select_model_different_strategies(self):
        ...
        model2 = self.create_test_model("reliable-model", reliability_score=0.95, cost_per_1k_tokens=0.005)
    ...
    def test_select_model_with_fallback(self):
        ...
        primary_model = self.create_test_model("primary-model", fallback_models=["fallback1", "fallback2"])
        ...
        registry.get_models_by_semantic_type.return_value = [primary_model, fallback1_model, fallback2_model]
    ...
    def test_select_model_with_fallback_execution(self):
        ...
        registry.query_models.return_value = [primary_model]
        registry.get_models_by_semantic_type.return_value = [primary_model, fallback_model]
        
        scorer.score_model.return_value = score
        scorer.rank_models.return_value = [(primary_model, score), (fallback_model, score)]
    ...
    def test_custom_strategy_with_weights(self):
        ...
        criteria = SelectionCriteria(strategy=SelectionStrategy.CUSTOM, custom_weights=custom_weights)
