...
class TestProviderAutoRegistry:
    ...
    def test_register_discovered_providers(self, auto_registry):
        ...
        with patch('gemini_sre_agent.llm.factory.LLMProviderFactory.register_provider') as mock_register:
    ...
    def test_validate_discovered_providers(self, auto_registry):
        ...
        with patch('gemini_sre_agent.llm.provider_framework.validator.ProviderValidator') as mock_validator:
            mock_validator.return_value.validate_provider_class.return_value = ["error1", "error2"]
...
class TestProviderCapabilityDiscovery:
    ...
    @pytest.mark.asyncio
    async def test_discover_provider_capabilities(self, capability_discovery, mock_provider):
        """Test capability discovery for a provider."""
        capabilities = await capability_discovery.discover_provider_capabilities(mock_provider)
    ...
    @pytest.mark.asyncio
    async def test_test_embeddings_capability(self, capability_discovery, mock_provider):
    ...
    def test_find_providers_with_capability(self, capability_discovery):
        ...
        streaming_providers = capability_discovery.find_providers_with_capability("streaming")
    ...
    def test_find_providers_matching_requirements(self, capability_discovery):
        ...
        matching = capability_discovery.find_providers_matching_requirements(["streaming", "tools"])
    ...
    def test_validate_provider_for_use_case(self, capability_discovery):
        ...
        validation = capability_discovery.validate_provider_for_use_case("provider1", "tools")
        assert validation["valid"] is True
        assert validation["score"] == 1.0
        
        validation = capability_discovery.validate_provider_for_use_case("provider1", "vision")
    ...
    def test_export_capability_report(self, capability_discovery):
        ...
        capability_discovery.capability_registry = {
            "provider1": {
                "streaming": MagicMock(supported=True, to_dict=lambda: {"name": "streaming", "supported": True}),
            },
        }
...
class TestProviderFrameworkIntegration:
    ...
    async def test_end_to_end_provider_creation(self):
        ...
        capabilities = await capability_discovery.discover_provider_capabilities(provider)
        assert "streaming" in capabilities
    ...
    def test_provider_line_count_requirement(self):
        ...
        simple_provider_path = Path(__file__).parent.parent / "llm" / "provider_framework" / "examples" / "simple_provider.py"
        ...
        if simple_provider_path.exists():
            ...
            assert len(code_lines) < 50, f"Simple provider has {len(code_lines)} lines, should be < 50"
