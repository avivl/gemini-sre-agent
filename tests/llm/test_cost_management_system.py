...
from gemini_sre_agent.llm.budget_manager import BudgetConfig, BudgetManager, BudgetPeriod, EnforcementPolicy
...
from gemini_sre_agent.llm.cost_management_integration import IntegratedCostManager, create_default_cost_manager
from gemini_sre_agent.llm.cost_optimizer import OptimizationConfig, OptimizationStrategy, CostOptimizer
...
class TestIntegratedCostManager:
    ...
    async def test_estimate_request_cost(self, integrated_manager):
        ...
        with patch.object(integrated_manager.cost_manager, 'estimate_cost') as mock_estimate:
            mock_estimate.return_value = 0.06
            
            cost = await integrated_manager.estimate_request_cost("openai", "gpt-4", 1000, 500)
    ...
    async def test_can_make_request(self, integrated_manager):
        ...
        with patch.object(integrated_manager.cost_manager, 'estimate_cost') as mock_estimate:
            ...
            can_make, message = await integrated_manager.can_make_request("openai", "gpt-4", 1000, 500)
    ...
    async def test_get_optimal_provider(self, integrated_manager):
        """Test getting optimal provider."""
        with patch.object(integrated_manager.cost_optimizer, 'get_optimal_provider') as mock_optimal:
            mock_optimal.return_value = ("openai", "gpt-4", 0.85)
            
            provider, model, score = await integrated_manager.get_optimal_provider("text", 1000, 500)
