...
def main():
    ...
    try:
        ...
        try:
            env_config = env_manager.load_config(temp_config_file)
            
            print(f"   ✅ Environment variables integrated successfully")
            print(f"   ML primary model name: {env_config.ml.primary_model.name}")
            print(f"   ML primary model max tokens: {env_config.ml.primary_model.max_tokens}")
            print(f"   ML primary model temperature: {env_config.ml.primary_model.temperature}")
            
            # Show secrets (masked)
            if env_config.secrets.gemini_api_key:
                print(f"   Gemini API key: {env_config.secrets.gemini_api_key.get_secret_value()[:10]}...")
    ...
    print("   This feature allows the configuration to be reloaded without restarting the application.")
