#!/usr/bin/env python3
"""
Comprehensive Dogfooding Demo
Runs all error scenarios and demonstrates the full SRE agent system.
"""
...
def run_comprehensive_scenarios():
    """Run all error scenarios in a comprehensive manner."""
    print("🔥 Running comprehensive error scenarios...")
    ...
    scenarios = [
        {
            "name": "Mathematical Errors",
            "endpoints": ["/error/division"] * 3,
            "description": "Test division by zero errors"
        },
        {
            "name": "Resource Errors", 
            "endpoints": ["/error/memory"] * 2,
            "description": "Test memory allocation errors"
        },
        {
            "name": "Network Errors",
            "endpoints": ["/error/connection"] * 2,
            "description": "Test connection errors"
        },
        {
            "name": "Validation Errors",
            "endpoints": ["/error/validation"] * 2,
            "description": "Test validation errors"
        },
        {
            "name": "Random Errors",
            "endpoints": ["/error/random"] * 5,
            "description": "Test random error scenarios"
        }
    ]
...
def analyze_logs():
            ...
            for line in lines:
                if line.strip():
                    ...
                    try:
                        ...
                        # Not a JSON log entry, check for keywords
    ...
    if error_count > 0:
        print(f"     ✅ Found {error_count} errors in agent logs")
    else:
        print(f"     ⚠️  No errors found in agent logs")
...
def check_patches():
    ...
    for patch_file in patch_files:
        try:
            ...
            print(f"       Priority: {patch_data.get('priority', 'Unknown')}")
        except Exception as e:
            print(f"     - {patch_file.name} (Error reading: {e})")

def main():
    """Main demo function."""
    print("🎭 SRE Agent Comprehensive Demo")
    print("=" * 60)
