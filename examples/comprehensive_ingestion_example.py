...
async def demo_queued_file_system():
    ...
    try:
        ...
        async for log_entry in adapter.get_logs():
            ...
            if processed_count >= 10 or (datetime.now(timezone.utc) - start_time).seconds >= 5:
                break
...
async def demo_log_manager():
    ...
    try:
        ...
        logger.info(f"Processed {processor.processed_count} logs with {processor.error_count} errors")
